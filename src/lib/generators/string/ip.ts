...
const ipv6_generator: Generator<z.$ZodIPv6> = (schema, ctx) => {
    ...
    const exact_length_too_long = length_constraints.exact != null && length_constraints.exact > IPV6_MAX_LENGTH;
    const exact_length_too_short = length_constraints.exact != null && length_constraints.exact < IPV6_MIN_LENGTH;
...
function partition(
    ...
    if (total < num_partitions * min_partition_size || total > num_partitions * max_partition_size) {
        throw new Error(`Cannot partition ${total} into ${num_partitions} Partitions, where each partition must be between ${min_partition_size} and ${max_partition_size}.`);
    }
