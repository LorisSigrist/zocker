...
const cidrv4_generator: Generator<z.$ZodCIDRv4> = (schema, ctx) => {
    ...
    const exact_length_too_short = length_constraints.exact && length_constraints.exact < CIDR_V4_MIN_LENGTH;
    const exact_length_too_long = length_constraints.exact && length_constraints.exact > CIDR_V4_MAX_LENGTH;

    if (max_length_too_short || exact_length_too_short || min_length_too_long || exact_length_too_long) {
        throw new InvalidSchemaException(`CIDRv4 must be between ${CIDR_V4_MIN_LENGTH} and ${CIDR_V4_MAX_LENGTH} characters long`);
    }
...
const cidrv6_generator: Generator<z.$ZodCIDRv6> = (schema, ctx) => {
    ...
    const exact_length_too_short = length_constraints.exact && length_constraints.exact < CIDR_V6_MIN_LENGTH;
    const exact_length_too_long = length_constraints.exact && length_constraints.exact > CIDR_V6_MAX_LENGTH;

    if (max_length_too_short || exact_length_too_short || min_length_too_long || exact_length_too_long) {
        throw new InvalidSchemaException(`CIDRv6 must be between ${CIDR_V6_MIN_LENGTH} and ${CIDR_V6_MAX_LENGTH} characters long`);
    }
    ...
    const mask: number = mask_length == 1
        ? faker.datatype.number({ min: 0, max: 9 })
        : mask_length == 2 ? faker.datatype.number({ min: 10, max: 99 })
