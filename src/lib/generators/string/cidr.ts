...
import { generateIPv4OfLength } from "./ip.js";
...
const cidrv4_generator: Generator<z.$ZodCIDRv4> = (schema, ctx) => {
    ...
    if(max_length_too_short || exact_length_too_short || min_length_too_long) {
...
const cidrv6_generator: Generator<z.$ZodCIDRv6> = (schema, ctx) => {
    const pattern =schema._zod.def.pattern ?? z.regexes.cidrv6;
