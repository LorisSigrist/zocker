...
const ulid_generator: Generator<z.$ZodULID> = (schema, ctx) => {
    ...
    const exact_length_wrong = length_constraints.exact != null && length_constraints.exact != ULID_LENGTH;
    const starts_with_too_long = content_constraints.starts_with.length > ULID_LENGTH;
    const ends_with_too_long = content_constraints.ends_with.length > ULID_LENGTH;
    const includes_too_long = content_constraints.includes.some(i => i.length > ULID_LENGTH);

    if(min_length_too_long || max_length_too_short || exact_length_wrong || starts_with_too_long || ends_with_too_long || includes_too_long) {
        throw new Error("Invalid length constraints for ULID. All valid ULIDs are 26 characters long");
    }

    let ulid = content_constraints.starts_with + content_constraints.includes.join("");
    const generated_length = ULID_LENGTH - ulid.length - content_constraints.ends_with.length;
