 ...
 * @throws InvalidSchemaException if the Schema cannot be satisfied
 */
const generate_cuid2: Generator<z.$ZodCUID2> = (
    schema,
    generation_context
) => {
    const length_constraints = getLengthConstraints(schema);
    ...
    // Validate that the length constraints can be satisfied for a CUID
    const exact_length_too_short = length_constraints.exact != null && length_constraints.exact < CUID2_MIN_LENGTH;
    const max_length_too_short = length_constraints.max < CUID2_MIN_LENGTH;
    if (exact_length_too_short || max_length_too_short) {
        throw new InvalidSchemaException("CUID2 cannot be less than 2 characters long");
    }

    const length = length_constraints.exact ?? faker.datatype.number({
        min: Math.max(length_constraints.min, CUID2_MIN_LENGTH),
        max: length_constraints.max == Infinity ? length_constraints.min + CUID2_COMMON_LENGTH : length_constraints.max
    });
...
export const CUID2Generator: InstanceofGeneratorDefinition<
    z.$ZodCUID2
> = {
